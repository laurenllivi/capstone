...
def find_venue(request):
    ...
    venues = hmod.Listing.objects.filter(geolocation__distance_lte=(pnt, D(mi=50))).filter(listing_type=venue_type)
    venue_pics_dict = {}
...
class FindVenueForm(forms.Form):
    ...
    venue_type = forms.ChoiceField(required=False, widget=forms.Select(), choices=choices.VENUE_TYPE_CHOICES)
